...
export default class SPRDDevice {
  ...
  async sendHello(): Promise<string> {
    ...
    return new TextDecoder("utf-8").decode(
      response.data.slice(0, response.data.byteLength - 1)
    );
  }
  ...
  private async receiveAck() {
    const response = await this.receivePacket();
    if (response.type !== HDLC_TYPE.REP_ACK)
  ...
  async receivePacket(): Promise<Packet> {
    ...
    do {
      ...
      console.log(
        "rcvBuf:",
        Array.prototype.map
          .call(this.rcvBuf.slice(0, this.rcvLen), (x) =>
    ...
    const data =
      dataLength > 0 ? new Uint8Array(buffer, 4, dataLength) : undefined;
    const crc = view.getUint16(4 + dataLength);
    const expectedCrc = this.bromCrc(array, 0, 4 + dataLength);
    if (crc !== expectedCrc)
      throw Error(`CRC mismatch: expected ${expectedCrc}, got ${crc}`);
  ...
  private async sendPacket(packet: Packet) {
    ...
    const buffer = new ArrayBuffer(
      SPRDDevice.HDLC_FRAME_MIN_SIZE * 2 + dataLength * 2
    );
    const view = new DataView(buffer);
    const array = new Uint8Array(buffer);
    ...
    if (!(await this.transferOut(buffer.slice(j, buffer.byteLength))))
      throw Error("Failed to transferOut");
  }
  ...
  private async transferIn(): Promise<boolean> {
    ...
    try {
      result = await this.device.transferIn(
        this.epIn,
        this.rcvBuf.byteLength - this.rcvLen
      );
    } catch (error) {
